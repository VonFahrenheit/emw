...
LoadNoviceShaman:
		STA !GFX_status+$0C
		JSL !GetVRAM					; > Get index
		LDA #$30 : STA.l !VRAMbase+!VRAMtable+$04,x	; > Source bank
		PHP						;\ Push processor and change to 16-bit
		REP #$20					;/
		LDA #$C408 : STA.l !VRAMbase+!VRAMtable+$02,x	;\
		LDA !GFX_status+$0C				; |
		AND #$00FF					; |
		ASL #4						; | Upload data
		ORA #$7000					; |
...
levelinitA:
	...
	;	LDA #$7000 : JSR PLANT_GFX
		...
		STZ $00
		STZ $01
		LDA #$94 : STA !GFX_status+$02		; > Plant base tile = 0x100
...
levelinitE:
	...
	.Shaman	LDA #$60 : JSR LoadNoviceShaman
		.NoShaman
...
levelinit10:
		...
		INC !SideExit
		LDA #$0C : STA !GFX_status+$09

		REP #$20

	;	LDA #$0100 : STA !BG2BaseV
...
level3:
	...
	+	SEP #$20				;/
		...
		.MakeTable
		STA !HDMA3source+1			; > Hi byte of table
	...
	-	DEC A : BPL -				; |
		...
		.Castle
	;	LDA !Level				;\ No castle on level 0x11
	;	CMP #$0011 : BEQ ..R			;/
		LDA $1E
		LSR #3
		SEP #$30				; > Regs 8 bit
...
levelD:
	...
	; all scratch RAM is used in this routine
	; $00 - scratch
	; $02 - for stretching ground section
	; $04 - for compressing ground section
	; $06 - pointer
	; $08 - scratch for storing index
	; $0A - holds HDMA table base index
	; $0C - for stretching ceiling section
	; $0E - scratch for ceiling parallax math

	; ceiling section is $140 bytes
	; dead zone + ground section are $1B8 bytes
	...
	+
		...
		LDA #$013F
		SEC : SBC $20
		SEC : SBC #$0040			; account for ceiling section (which is always 64 scanlines)
		LSR A
		STA $40A000+$140,x
		BCC $01 : INC A
		STA $40A005+$140,x
		CLC : ADC $40A000+$140,x
		STA $00

		LDA $20					;\
		STA $40A003+$140,x			; | Set up Y coords for non-parallax part
		STA $40A008+$140,x			;/

		PEI ($20)				; preserve BG2 Y for ground section

		STZ $0C					;\
		LDA $1C					; |
		CMP #$00C0				; \ cap at 0x0C0
		BCC $03 : LDA #$00C0			; /
	-	CMP #$000C : BCC +			; |
		SBC #$000C				; |
		INC $0C					; | starting factor index
		BRA -					; |
	+	LDA $0C					; |
		CLC : ADC #$0000			; |
		TAY					;/
	...
	+	PLA : STA $20				; restore BG2 Y for ground section
		...
		CLC : ADC $0A
		TAX
		LDA #$0000 : STA $40A00E+$140,x		; [actual end]-1 so I don't overstep (16-bit mode)
		LDA #$0001				;\
	-	STA $40A00A+$140,x			; |
		DEX #5					; | set up scanline counts
		DEY : BPL -				; |
...
..ScrollTable
db $00,$F7,$EE,$E6,$DD,$D6,$CE,$C7,$C0,$B9,$B3,$AC,$A6,$A1,$9B,$96
db $90,$8B,$86,$82,$7D,$79,$74,$70,$6C,$69,$65,$61,$5E,$5B,$57,$54
...
; .HDMA if !BossData+0 is 6 or greater
	...
	MinorExSpriteSlot:			; if it returns negative, there are no empty slot
		LDX #$0B
	-	LDA $77F0,x : BEQ .Yes
		DEX : BPL -
	.Yes	RTS
...
SpeedPlatform:
	...
	+	CLC : ADC $1C				; |
		...
		LDA $14
		AND #$03 : BNE ..Return
		JSR MinorExSpriteSlot
		BMI ..Return

		JSR DebrisRNG

		LDA #$01 : STA $77F0,x			; number
		LDA #$50
		...
		CLC : ADC $01
		STA $7808,x				; X lo
		LDA #$01 : STA $7814,x			; Y hi
		LDA #$0C : STA $78EA,x			; X hi
