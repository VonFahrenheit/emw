...
CREATE_RAM_CODE:
	...
	.Start	STA $41800B,x				; > Upload size
		PHA
		...
		STA $41800F,x				; |
		XBA : STA $418009,x			;/ (inverted so we don't overwrite offset $0B)
		...
		LDA #$A921 : STA $418014,x		;/ (value for LDA is stored later at offset $16)
		...
		AND #$00FF				; | Bank
		STA $418006,x				;/
		LDA !VRAMtable+$05,y : BMI .Read	; determine transfer direction


	.Write	STA $418010,x				; > VRAM destination
		LDA #$1801 : STA $418016,x		; > DMA mode
	...
	.Read	LDA #$3981 : STA $418016,x		; > DMA mode
...
ReturnToNMI:	REP #$20				; > A 16-bit
		LDX #$00 : PHX : PLB			; > Set bank

		LDX !MsgTrigger : BNE +			; > let message boxes use indirect mode
		LDX !GameMode				;\ let title screen use indirect mode
		CPX #$0B : BCC +			;/
		LDA !HDMA2source : STA $4322		;\
	+	LDA !HDMA3source : STA $4332		; |
		LDA !HDMA4source : STA $4342		; | HDMA source mirrors to prevent tearing
		LDA !HDMA5source : STA $4352		; |
		LDA !HDMA6source : STA $4362		; |
		LDA !HDMA7source : STA $4372		;/
...
.Custom		db $01,$06,$04,$05,$04,$08,$10,$01,$11,$10,$04,$03,$01,$04,$04,$02	; 0X
		...
		db $16,$0D,$02,$05,$05,$05,$05,$06,$06,$06,$16,$06,$00,$00,$00,$00	; 2X
