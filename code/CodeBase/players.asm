...
; output:
;	C = contact flag (0 = no contact (BCC), 1 = contact (BCS))
...
; _Y checks for contact with player based on index, Y is used as input for this one
	...
	.P1
		...
		LDA $E8
		CLC : ADC $EC
		CMP !P2HurtboxX-$80 : BMI ..nocontact
		LDA !P2HurtboxH-$80
		AND #$00FF
		...
		LDA $EA
		CLC : ADC $EE
		CMP !P2HurtboxY-$80 : BMI ..nocontact
		..contact
		SEC
		SEP #$20
		RTL
	...
	.Y
		...
		LDA $E8
		CLC : ADC $EC
		CMP !P2HurtboxX-$80,y : BMI ..nocontact
		LDA !P2HurtboxH-$80,y
		AND #$00FF
		...
		LDA $EA
		CLC : ADC $EE
		CMP !P2HurtboxY-$80,y : BMI ..nocontact
		..contact
		SEC
		SEP #$20
		RTL
...
; output: void
	...
	.Kill
		...
		LDA #$C0 : STA !P2YSpeed-$80,y		; set Y speed
		LDA #$88 : STA !P2ShowHP-$80,y		; set this immediately since it interacts with KNOCKED_OUT
		CPY #$80 : BEQ ..p2
		..p1
		REP #$20
		LDA !P1DeathCounter
	...
	.Mario
		...
		STA !P2FlameStarCharge-$80,y		; interrupt ult charge
		RTS
...
; input: X = fusion sprite index
; output: fusion sprite is added to projectile list
	RegisterProjectile:
		LDY !PProjectile_Index
		TXA : STA !PProjectile_List,y
		INC !PProjectile_Index
		RTL

; input: X = fusion sprite index
; output: fusion sprite is removed from projectile list (and list is updated)
	RemoveProjectile:
		...
		..loop
		DEY : BMI .Done
		CMP !PProjectile_List,y : BNE ..loop

		.AdjustList
		DEC !PProjectile_Index
		..loop
		CPY !PProjectile_Index : BEQ .Done
		LDA !PProjectile_List+1,y : STA !PProjectile_List,y
		INY : BRA ..loop
