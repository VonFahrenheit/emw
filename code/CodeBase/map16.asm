...
; input:
;	X = 16-bit X coordinate
;	Y = 16-bit Y coordinate
; output:
;	A = 16-bit acts like setting
;	A returns 16-bit, index returns 8-bit
;	X/Y registers are shredded
;	$00 = 24-bit acts like table pointer (lo block or hi block)
;	$03 = 16-bit raw map16 tile number
;	$9A = 16-bit X position of map16 tile (can be used between calls)
;	$98 = 16-bit Y position of map16 tile (can be used between calls)
;
; _Sprite version uses sprite-relative input coords
; _Tile version uses $9A/$98 as inputs, regs do not matter
;
; NOTE: only the lo byte of X is maintained
;	hi byte of X is ALWAYS shredded by this routine!
