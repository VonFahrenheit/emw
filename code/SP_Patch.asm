...
pullpc
	...
	.BG3	STA.l !VRAMbase+!VRAMtable+$05,x	; > Dest VRAM
		...
		LDA $C7FE				; | Source address
		CLC : ADC #$8BBF			; |
		STA.l !VRAMbase+!VRAMtable+$02,x	;/
		PHX					;\
		LDX $C7FE				; > $7FC7FE holds the index to this table
		LDA $8381,y : STA $418BBF,x		; | Copy upload data to safe RAM
		LDA $8383,y : STA $418BC1,x		; |
		LDA $8385,y : STA $418BC3,x		; |
		LDA $8387,y : STA $418BC5,x		; |
		TXA					; \
		CLC : ADC #$0008			;  > Update index to backup table
		STA $C7FE				; /
		PLX					;/
		LDA $837F,y				;\
