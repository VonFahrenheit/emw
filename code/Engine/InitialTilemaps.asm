...
;=====================================================================
;
; THIS IS THE PART RESPONSIBLE FOR GENERATING INITIAL TILEMAPS
;
;=====================================================================

	GetInitialTilemaps:
		SEP #$30					; all regs 8-bit
		LDA.b #.GenerateLevelBG1 : STA $3180		;\
		LDA.b #.GenerateLevelBG1>>8 : STA $3181		; | wait for SA-1 to get BG1 level data
		LDA.b #.GenerateLevelBG1>>16 : STA $3182	; |
		JSR $1E80					;/
	...
	; for BG2, we need to check if it should be level data or a background
		REP #$10					;\
		LDX !Level					; |
		LDA.l !Layer2Type,x				; | determine layer 2 type
		SEP #$30					; > all regs 8-bit
		LDX #$02					; > X = DMA bit
		AND #$02 : BNE .GenerateBackgroundBG2		;/

	.BG2level
		LDA.b #.GenerateLevelBG2 : STA $3180		;\
		LDA.b #.GenerateLevelBG2>>8 : STA $3181		; | wait for SA-1 to get BG2 level data
		LDA.b #.GenerateLevelBG2>>16 : STA $3182	; |
		JSR $1E80					;/
		REP #$20					;\
		LDA !BG2Address : STA $2116			; |
		LDA.w #!DecompBuffer : STA $4312		; | upload BG2 tilemap
		LDA #$1000 : STA $4315				; |
		STX $420B					;/
		SEP #$30					;\ return
		RTL						;/
	...
	.GenerateBackgroundBG2
		LDA.b #DecompressBackground : STA $3180		;\
		LDA.b #DecompressBackground>>8 : STA $3181	; | wait for SA-1 to get BG2 background data
		LDA.b #DecompressBackground>>16 : STA $3182	; |
		JSR $1E80					;/
		...
		LDA $20						;\
		SEC : SBC #$0008				; | see if simple or multi upload is needed
		AND #$00F8 : BNE ..multi			;/
		..simple					;\
		LDA $20						; |
		SEC : SBC #$0008				; |
...
;===========================================;
; read layer 1 level data to make a tilemap ;
;===========================================;
	.GenerateLevelBG1
...
;===========================================;
; read layer 2 level data to make a tilemap ;
;===========================================;
	.GenerateLevelBG2
		PHB : PHK : PLB					;\ wrapper start
		PHP						;/
...
;=====================================================================
;
; THIS IS THE PART RESPONSIBLE FOR DECOMPRESSING THE BACKGROUND
;
;=====================================================================
	!SourceIndex	= $03
	!OutputIndex	= $05
	...
	+	STA $00						;/
		...
		INY #2						; |
		LDA [$07],y : STA.w !BG2Tilemap+$42,x		; |
		INX #4						; | get map16 tilemap data
		TXA						; |
		AND #$003F : BNE ..samerow			; |
		..newrow					; |
		TXA						; |
		CLC : ADC #$0040				; |
		TAX						; |
		..samerow					; |
		STX !OutputIndex				;/
		CPX #$2000 : BCC ..loop				; loop until 8KB of data has been transcribed
