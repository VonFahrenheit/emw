...
; these codes replace Lunar Magic's GFX/tilemap loaders
	...
	GFX_Loader:
		...
		LDA !Level : TAX				;\ X = level index
		ASL #5 : TAY					;/ Y = GFX list index
	...
	;	ORA $08						; |
		...
		LDA.w .BG3Speed,x : STA !BG3XSpeed		; |
		..done						;/
		...
		LSR #3						; |
		TAX						; |
		LDA.w .BG3Speed,x : STA !BG3YSpeed		; |
		..done						;/
		...
		AND #$2000 : BEQ ..notilemap			;/
		LDA [$03],y					;\
		AND #$0FFF					; | check for BG3 tilemap
		CMP #$007F : BEQ ..notilemap			;/
		PHA						;\
		PHX						; |
		...
		LDA #$80 : STA $2115				;\
		REP #$20					; |
		LDA #$1908 : STA $4310				; | set up DMA to wipe odd bytes of BG3 tilemap
		LDA.w #.Some0014+1 : STA $4312			; |
		LDA.w #(.Some0014+1)>>8 : STA $4313		;/
		..upload					;\
		LDA #$1000 : STA $4315				; | upload
		LDA.w .VRAM+8,x : STA $2116			;/
...
; FOR REFERENCE: 2bpp -> 4bpp
;		CPX #$0010 : BCC ..noconversion
;		CPX #$0060 : BCS ..noconversion
;		CMP #$0028 : BCC ..noconversion
;		CMP #$002B+1 : BCS ..noconversion
;		JSL !DecompressFile
;		PHB
;		PHX
;		PHY
;		SEP #$20
;		LDA.b #!DecompBuffer>>16 : PHA : PLB
;		REP #$20
;		LDY #$0FE0
;		TYA
;	-	ASL A
;		TAX
;		LDA.w !DecompBuffer+$00,y : STA !DecompBuffer+$00,x
;		LDA.w !DecompBuffer+$02,y : STA !DecompBuffer+$02,x
;		LDA.w !DecompBuffer+$04,y : STA !DecompBuffer+$04,x
;		LDA.w !DecompBuffer+$06,y : STA !DecompBuffer+$06,x
;		LDA.w !DecompBuffer+$08,y : STA !DecompBuffer+$08,x
;		LDA.w !DecompBuffer+$0A,y : STA !DecompBuffer+$0A,x
;		LDA.w !DecompBuffer+$0C,y : STA !DecompBuffer+$0C,x
;		LDA.w !DecompBuffer+$0E,y : STA !DecompBuffer+$0E,x
;		STZ.w !DecompBuffer+$10,x
;		STZ.w !DecompBuffer+$12,x
;		STZ.w !DecompBuffer+$14,x
;		STZ.w !DecompBuffer+$16,x
;		STZ.w !DecompBuffer+$18,x
;		STZ.w !DecompBuffer+$1A,x
;		STZ.w !DecompBuffer+$1C,x
;		STZ.w !DecompBuffer+$1E,x
;		TYA
;		SEC : SBC #$0010
;		TAY
;		BPL -
...
;		..noconversion
		...
		LDA.w #!DecompBuffer : STA $4312		; |
		LDA.w #!DecompBuffer>>8 : STA $4313		; |
		LDA.w .FileSize,x : STA $4315			; | DMA file to VRAM
		LDA.w .VRAM,x : STA $2116			; |
		SEP #$20					; |
		LDA #$02 : STA $420B				; |
		REP #$20					;/
		..nodecomp					;\
...
.Vanilla
	...
	%gfx_index(BonyBeetle)		; 031
	%gfx_index(DryBones)		; 032
...
PalsetDefaults:
	...
	; stuff loaded with players
		...
		LDA !Characters					; |
		AND #$0F : BNE +				; |
		LDA #$28 : BRA ++				; | mario fireball can be included in mario's file
	+	LDA !Characters					; |
	...
	STA !SD_Mark+!SD_LuigiWallRun_offset
		.NoLuigiFire					;/
...
.VolcanoLotus	%cmd($F37, $00, $04, VolcanoLotus, 0)
		...
		db $FF
;===============================================================
.BooBlock	%cmd($F3D, $0C, $04, BooBlock, 2)
		%mark(Boo)
		db $FF
;===============================================================
.BigBoo		%cmd($F3C, $00, $20, BigBoo, 2)
...
.ParaKoopa	%cmd($F00, $00, $0C, ParaKoopa, 2)
		...
		db $FF
;===============================================================
.ParaKoopaBlue	%cmd($F00, $00, $0C, ParaKoopaBlue, 2)
...
; 0A: bytes used in $7E2000-$7EACFF (35 KB)
	...
	; handle commands
		...
		SEP #$20					;/
		LDA ($00),y					; read first byte of header (command)
		AND #$7F : BEQ ..LoadChunk			; 00 = load chunk from decomp buffer
		CMP #$01 : BEQ ..RotateInit			; 01 = rotate
		CMP #$02 : BEQ ..ScaleInit			; 02 = scale
...
; !BigRAM+$12	- UNUSED
; !BigRAM+$14	- UNUSED
; !BigRAM+$6A	- 0 = load from working buffer, 1+ = load from decomp buffer
...
; NOTE: only SNES can call !DecompressFile, so it can't be done here
	...
	; copies a chunk from decomp buffer to working buffer
	..LoadChunk
		REP #$20
		INY
	...
	--	LDA !BigRAM+$6A : BEQ +			;\
		LDA.w #!DecompBuffer>>8 : STA $0C-1	; |
		LDA.w #!DecompBuffer : BRA ++		; |
	+	LDA $0F-1 : STA $0C-1			; | get read address
		LDA !BigRAM+$7A				; |
	++	CLC : ADC $2306				; |
		STA $0A					;/
		JSR ChunkToCache			; get chunk
...
macro rotate(source, chunk, iterations, copies, angle)
...
macro scale(source, chunk, iterations, x, y)
	if <source> == !src_buffer
		db $02
		else
		db $82
...
; NOTE:
;	!src_decomp can only be used if the file has the same width encoding as the output chunks
;	!src_buffer can always be used, since %loadchunk() translates the width encoding
...
%rotate(!src_buffer, $00, 1, 15, $20)	; rotate: chunk 0, iterations 1, copies 15, angle $20
..end
db $FF					; end file
...
%rotate(!src_buffer, $10, 1, 15, $20)	; rotate: chunk 16, iterations 1, copies 15, angle $20
%scale(!src_buffer, $00, 16, $E0, $E0)	; scale: chunk 0, iterations 16, x 82%, y 82%
%scale(!src_buffer, $00, 16, $C0, $C0)	; scale: chunk 0, iterations 16, x 75%, y 75%
..end
db $FF					; end file
...
%rotate(!src_buffer, $00, 4, 1, $10)	; rotate: chunk 0, iterations 4 (chunks 0-3), copies 1, angle $10
%rotate(!src_buffer, $00, 4, 1, $20)	; rotate: chunk 0, iterations 4 (chunks 0-3), copies 1, angle $20
%rotate(!src_buffer, $00, 4, 1, $30)	; rotate: chunk 0, iterations 4 (chunks 0-3), copies 1, angle $30
%rotate(!src_buffer, $00, 4, 1, $40)	; rotate: chunk 0, iterations 4 (chunks 0-3), copies 1, angle $40
..end
db $FF					; end file
...
..header
dw $0010				; width encoding
%size($200*24)				; size: 16 32x32 chunks
dw $E0B					; source ExGFX
db !SD_LuigiWallRun_offset		; SD GFX status index
db $10,$20				; chunk dimensions (32x32)
..commands
%rotate(!src_decomp, $00, 1, 1, $20)	; rotate: chunk 0, iterations 1, copies 1, angle $20
%rotate(!src_decomp, $01, 1, 1, $40)	; rotate: chunk 1, iterations 1, copies 1, angle $40
%rotate(!src_decomp, $02, 1, 1, $60)	; rotate: chunk 2, iterations 1, copies 1, angle $60
%rotate(!src_decomp, $03, 1, 1, $20)	; rotate: chunk 3, iterations 1, copies 1, angle $20
%rotate(!src_decomp, $04, 1, 1, $40)	; rotate: chunk 4, iterations 1, copies 1, angle $40
%rotate(!src_decomp, $05, 1, 1, $60)	; rotate: chunk 5, iterations 1, copies 1, angle $60
%rotate(!src_decomp, $06, 1, 1, $20)	; rotate: chunk 6, iterations 1, copies 1, angle $20
%rotate(!src_decomp, $07, 1, 1, $40)	; rotate: chunk 7, iterations 1, copies 1, angle $40
%rotate(!src_decomp, $08, 1, 1, $60)	; rotate: chunk 8, iterations 1, copies 1, angle $60
%rotate(!src_decomp, $09, 1, 1, $20)	; rotate: chunk 9, iterations 1, copies 1, angle $20
%rotate(!src_decomp, $0A, 1, 1, $40)	; rotate: chunk 10, iterations 1, copies 1, angle $40
%rotate(!src_decomp, $0B, 1, 1, $60)	; rotate: chunk 11, iterations 1, copies 1, angle $60
%rotate(!src_decomp, $00, 12, 1, $80)	; rotate: chunk 0, iterations 12 (all chunks), copies 1, angle $80
..end
db $FF					; end file
