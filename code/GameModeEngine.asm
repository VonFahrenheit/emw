...
; TO DO:
; - remove unused modes
; - reorder modes and have ALL gamemode writes be done via defines so they can be easily moved around
	...
	RunGameMode:
		...
		.GameModePtr
		dw GAMEMODE_00		; 00 - LOAD PRESENTS SCREEN
		dw PresentsScreen	; 01 - PRESENTS SCREEN - animation
		dw GAMEMODE_02		; 02 - PRESENTS SCREEN - fade out
		dw GAMEMODE_03_Main	; 03 - LOAD MAIN MENU - load main menu level data + initialize a bunch of RAM
		dw GAMEMODE_04		; 04 - ----
		dw GAMEMODE_05		; 05 - MAIN MENU - fade in to title screen
		dw GAMEMODE_06		; 06 - ----
		dw MainMenu		; 07 - MAIN MENU - "press any button"
		dw MainMenu		; 08 - MAIN MENU - actual menu
		dw GAMEMODE_09		; 09 - ----
		dw GAMEMODE_0A		; 0A - ----
		...
		dw GAMEMODE_10		; 10 - LOAD LEVEL - load intro text
		dw CallGameMode11	; 11 - LOAD LEVEL - actually load level
		dw CallGameMode12	; 12 - ----
...
;=============;
	...
	-	STA $400000+!MsgRAM,x
		...
		LDX #$8000 : STX $4300					;\
		LDX.w #MPU_PROGRAM_START : STX $4302			; |
		LDA.b #MPU_PROGRAM_START>>16 : STA $4304		; |
		LDX.w #MPU_PROGRAM_END-MPU_PROGRAM_START : STX $4305	; | DMA MPU program into place
		LDX.w #$1000 : STX $2181				; |
		STZ $2183						; |
		LDA #$01 : STA $420B					;/
...
;=============;
	...
	-	STA $6101,x
		...
		LDA #$C7 : STA !LevelEntry			;\ which level should be loaded (main menu)
		STZ !LevelEntry+1				;/
		JSR CallGameMode11				; load level
...
;==========================;
	...
	PLB
		LDA !GameMode
		CMP #$0F : BEQ .ProcessModules
		CMP #$13 : BEQ .ProcessModules
		RTS
...
;====================================;
	GAMEMODE_10:
		...
		STZ $1F0C					; | no HDMA
		STZ $420C					;/
		LDA !LevelTable1+$00 : BPL +			; never use intro text until after intro level
		LDA !DoorCounter : BEQ .IntroText		; otherwise, use it on first sublevel
