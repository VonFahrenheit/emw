...
; what do these modes actually have to do?
; - clear some misc RAM
; - load level
;	- level number
;	- layer 1 data (pointer at $05E000)
;		- primary header
;			- BG palette
;			- length of level
;			- back area color
...
;			yyyy	main entrance Y position, lo 4 bits
;		- $05F200
;			33	layer 3 setting
;			AAA	main entrance action (stand still, pipe, etc)
;			xxx	main entrance X position, lo 3 bits
;		- $05F400
;			MMMM	midway entrance screen number
;			ff	main entrance FG initial position (if auto-coord is off)
;			bb	main entrance BG initial position (if auto-coord is off)
;		- $05F600
;			[cut]	disable no yoshi intro flag
;			[cut]	unused flag
;			[cut]	vertical positioning flag
;			EEEEE	main entrance screen number
;		- $05DE00
;			I	ice level flag
;			W	water level flag
;			P	flag to use X/Y position method 2
;			XX	main entrance X position, hi 2 bits
;			s	smart spawn flag
;			SS	sprite spawn range
;		- $06FC00
;			0	set BG relative to FG flag
;			F	hi bit of Fffbb (FG/BG relative to player) if R is set
;			YYYYYY	main entrance Y position, hi 6 bits
;		- $06FE00
;			R	auto-coord flag (BG1 Y = entrance coord - 16*Fffbb)
;			L	main entrance face left flag
;			[cut]	unused flag
;			HHHHH	BG height -1 of 0 clear, relative BG offset if 0 is set
;
;		- read3(read3($05D9A2)+$5C) (the doc i have says "+70" but that is incorrect)
;			T	flag that layer 2 or layer 3 uses map16
;			B	flag to show bottom row of level
;			N	new sprite system flag
;			LLLLL	horizontal level mode
;
;	- midway entrances
;		- read3(read3($05D9E4)+$0A)+$000
;			I	ice level flag
;			W	water level flag
;			S	separate setings for midway entrance flag (if clear, only M bit is used)
;			M	highest bit of midway entrance screen number
;			Y	highest bit of midway entrance Y position
;			AAA	midway entrance action
;
;		- read3(read3($05D9E4)+$0A)+$200
;			yyyy	midway entrance Y position, lo 4 bits
;			xxxx	midway entrance X position, lo 4 bits
;
;		- read3(read3($05D9E4)+$0A)+$400
;			R	auto-coord flag (BG1 Y = entrance coord - 16*Fffbb)
;			L	midway entrance face left
;			[cut]	redirect midway entrance to other level
;			ff	midway entrance FG initial position (if auto-coord is off)
;			bb	midway entrance BG initial position (if auto-coord is off)
;
;		- read3(read3($05D9E4)+$0A)+$600
;			F	hi bit of Fffbb (FG/BG relative to player) if R is set
;			YYYYYY	midway entrance Y position, hi 6 btis
;
;	- layer 2 data (pointer at $05E600)
;		handled by DecompressBackground
;		table at $0EF310 holds additional layer 2 data
;			BBBB	map16 bank to use for layer 2 (if tilemap), or hi byte for all BG map16 tiles (if object)
;			[cut]	unused flag
;			h	indicates usage of hi nybble (0 = don't use, 1 = use)
;			D	data type (0 = object, 1 = tilemap)
;			[cut]	unused flag
;
;	- sprite data (pointer at $05EC00)
;		handled by Fe26
;
;	- ExAnimation data
;	- ExGFX data
;		handled by GFX_Loader and GFXIndex
;
;	- palette data
;		pointer table at $0EF600, points to raw color data
;		color data is 514 (not 512) words long (1028 bytes)
;		the first word is for !2132_RGB, the rest is a copy of the palette, starting at color 0
;
; - initialize players
; - initialize sprites
; - spawn held items
; - load light points
; - initialize palette systems
;	- HSL
;	- global light
;	- shader
; - clear cutscene RAM
; - clear misc long RAM flags
; - initialize camera
...
; code at $0583B8 ("LoadAgain" in all.log) is responsible for loading object data
; it ends with PLP : RTS, but that could easily be changed if needed
; most likely, it is better to go to $0583B5 so the JSR to $0581FB is included (sets up slope data)
...
; LOAD_LEVEL + camera setup -> DecompressBackground
; GM11 finisher -> GFX_Loader -> GFXIndex -> DecompressBackground

; so i think the way i want to do this is:
; 1 run LOAD_LEVEL and keep the first 16 DP values
; 2 let GM11 finisher run
; 3 let GM12 run GFX_Loader and GFXIndex
; 4 run Camera_Init
; 5 run level init
; 6 restore the 16 DP values from GM11's LOAD_LEVEL and run DecompressBackground
		...
		; TODO: figure out what this routine does
		;	don't need to transcribe or rewrite, just research its purpose
		PHK : PEA.w .Return-1
		PEA.w $A59B-1
		LDY #$01 : JML $00A6CC
		.Return
	...
	GAMEMODE_12:
		...
		PEI ($E2)						; |
		PEI ($E4)						; |
		PEI ($E6)						; | safely preserve this on stack
		PEI ($E8)						; |
		PEI ($EA)						; |
		PEI ($EC)						; |
		PEI ($EE)						;/
...
; ID = 0xFF	-> not native to this level
	...
	-	STZ $15,x					; |
		...
		LDA !Translevel : BEQ ..yoshicoindone		; unless level = 0, despawn collected yoshi coins
		STZ $2250					;\
		REP #$30					; |
...
; if sublevel number is $FFFF, the Yoshi Coin does not exist
	.DestroyCoin
		...
		TAX						;\
		SEP #$20					; |
		LDA #$25 : STA $40C800,x			; | erase yoshi coin
		LDA #$00 : STA $41C800,x			; |
		REP #$20					;/
		..return					;\
		TYA						; | update index
...
;	As soon as it returns, A is stored to $0F, making $0E hold the 16-bit level number to be loaded.
	...
	; LM coordinate initializer
	org read3($05DA17+1)			; hijack this LM routine from inside (LM goes crazy if you repoint te JSL)
	; repair
		SEP #$30
		LDA $5B
	pullpc
