	...
	-	STA $400000+!MsgRAM,x				; | clear MSG RAM
		...
		REP #$20					;\
		STZ !RAMcode_flag				; | kill any RAM code left over from level
		STZ !RAMcode_offset				; |
		SEP #$20					;/
	...
	; 80+ -> normal overworld
		...
		LDA #$F0 : STA $6DB0
		LDA #$10 : STA !GameMode
		LDA.b #!IntroLevel_UnexploredHill : STA !LevelEntry		; intro level (translevel num... but only kind of...)
		LDA.b #!IntroLevel_UnexploredHill>>8 : STA !LevelEntry+1	; set hi bit of intro level num
		LDA !LevelTable1+$00
		AND #$40 : TSB !LevelEntry+1
		STZ !Translevel					;
		LDA #$81					;\
	-	BIT $4212 : BMI - : BVS -			; | wait for no blanks, then enable NMI + auto joypad
		...
		LDA #$F0 : STA $6DB0
		LDA #$10 : STA !GameMode
		LDA.b #!IntroLevel_Airship : STA !LevelEntry		; intro level (translevel num... but only kind of...)
		LDA.b #!IntroLevel_Airship>>8 : STA !LevelEntry+1	; set hi bit of intro level num
		STZ !Translevel					;
		LDA #$81					;\
	-	BIT $4212 : BMI - : BVS -			; | wait for no blanks, then enable NMI + auto joypad
		STA $4200					;/
	...
	; $00A0B0 is the actual load overworld code in all.log (not that it's helpful here lmao)
		.LoadOverworld
	;	JSL !KillOAM
		STZ $2133
		STZ $2106
		...
		LDA #$40 : STA $210B				; BG1 chr address = 0x0000, BG2 chr address = 0x8000
	...
	;	STZ $2131
		...
		LDA #$3000 : STA !BG1Address			; BG1 tilemap address
		LDA #$3800 : STA !BG2Address			; BG2 tilemap address
		STZ $1A						;\
		STZ $1C						; |
		...
		LDA #$31					;\
		STA !2107					; | BG1 tilemap: address $3000 (0x6000), size 64x32
		STA $2107					;/
		LDA #$39					;\
		STA !2108					; | BG2 tilemap: address $3800 (0x7000), size 64x32
		STA $2108					;/
	...
	-	STA !DecompBuffer+$100,x
		...
		LDA.w #!DecompBuffer+$2000>>8 : STA $01		; | decompress a BG3 tilemap
		LDA.w #$B09 : JSL !DecompressFile		;/


		LDA.w #!DecompBuffer : STA $00			;\
		LDA.w #!DecompBuffer>>8 : STA $01		; | decompress a 2bpp GFX file
	...
	-	LDA.w !DecompBuffer+$2000,x					; |
		AND #$00FF							; |
		CMP #$0014 : BNE +						; | translate tile numbers
		LDA #$00FF : BRA ++						; > 0x014 -> 0x0FF
	+	LDA.w !DecompBuffer+$2000,x					; |
		AND #$FF7F							; > otherwise just subtract 0x80
	...
	-	LDA.w !DecompBuffer+$00,y : STA.w !DecompBuffer+$1000,x		; |
		...
		STZ.w !DecompBuffer+$1016,x					; | translate 2bpp GFX to 4bpp
		...
		LDA #$1000 : STA $4315				; | upload BG2 main playfield tilemap
		LDA !BG2Address : STA $2116			; | ($7000/$3800)
		STX $420B					;/


		LDA.w #!DecompBuffer+$1000 : STA $4312		;\
		LDA.w #!DecompBuffer+$1000>>8 : STA $4313	; |
		LDA #$1000 : STA $4315				; | upload BG2 main playfield GFX
		LDA #$4000 : STA $2116				; | ($8000/$4000)
		STX $420B					;/
	...
	..debugdone
		...
		LDA.w #!DecompBuffer>>8 : STA $4313		; | ($B000/$5800)
		LDA #$1000 : STA $4315				; |
		STX $420B					;/
		...
		LDA.w #!DecompBuffer>>8 : STA $4313		; | ($F000/$7800)
		LDA #$1000 : STA $4315				; |
		STX $420B					;/
...
;	- IGNORE all other cases!
	...
	.InitWindow
		SEP #$30
		LDA #$22 : STA !2123		;\ hide BG1/BG2 inside window, show BG3 ONLY inside window
		LDA #$03 : STA !2124		;/
		STZ !2125			; > enable sprites within window
...
;
	...
	.End
		...
		STZ $0214,x
		STZ $022A,x
		LDA #$2C : STA $0211,x					; BG2 tilemap address setting, HUD
		STA !2108
		LDA #$38 : STA $0213,x					; BG2 tilemap address setting, main playfield
		REP #$20
		LDA #$0012
		STA $0201,x
		STA $0203,x
