...
; VRAM map:
; 0x0000 / $0000	23.5 KiB BG1 GFX	376 8bpp characters (128x192 px)
; 0x5E00 / $2F00	512 B BG2 tilemap	1 tilemap, 32x8 tiles, HUD part
; 0x6000 / $3000	4 KiB BG1 tilemap	2 tilemaps, 64x32 tiles
; 0x7000 / $3800	4 KiB BG2 tilemap	2 tilemaps, 64x32 tiles, main part
; 0x8000 / $4000	8 KiB BG2 GFX		256 4bpp characters (128x128 px), main part
; 0xA000 / $5000	4 KiB BG2 GFX		128 4bpp characters (256x32 px), software-rendered HUD
; 0xB000 / $5800	4 KiB BG2 GFX		128 4bpp characters (256x32 px), character select chunk
; 0xC000 / $6000	16 KiB sprite GFX	512 4bpp characters (128x256 px)

; note that on the overworld, BG2 chr address = 0x8000 / $4000
; the main playfield tiles are accessed as num 0x000-0x0FF
; the HUD tiles are accessed as num 0x100-0x17F
; the character select tiles are accessed as num 0x180-0x1FF
...
; (make sure to exclude the HUD)
	...
	; X still has to be translevel num here
		...
		BIT !LevelTable1,x : BVC ..nocheckpoint			;\ check which entrance to use
		LDA !MapCheckpointTargetX : BNE ..checkpoint		;/
		..nocheckpoint						;
		LDY #$00						;\
		LDA !Translevel						; |
		CMP #$25						; | main entrance data
		BCC $03 : SBC #$24 : INY				; |
		STA !LevelEntry						; |
		STY !LevelEntry+1					;/
		BRA ..save						; go to shared
		..checkpoint						;\
		LDA !LevelTable2,x : STA !LevelEntry			; |
		LDA !LevelTable1,x					; | checkpoint entrance data
		AND #$20						; |
		BEQ $02 : LDA #$01					; |
		ORA #$40 : STA !LevelEntry+1				;/
		..save							;\ auto-save
		JSL !SaveGame						;/
