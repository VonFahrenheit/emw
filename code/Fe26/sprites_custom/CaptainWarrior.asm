...
CaptainWarrior:
	...
	if !Debug = 0
		...
		REP #$20						;\
		STZ !SpriteStatus+$00 : STZ !SpriteStatus+$02		; |
		STZ !SpriteStatus+$04 : STZ !SpriteStatus+$06		; |
		STZ !SpriteStatus+$08 : STZ !SpriteStatus+$0A		; | despawn all other sprites
		STZ !SpriteStatus+$0C : STZ !SpriteStatus+$0E		; |
		SEP #$20						; |
		LDA #$08 : STA !SpriteStatus,x				;/ > go to MAIN
	...
	-	LDA.w BodyPal,x : STA !ShaderInput+($A1*2),x		; | palette to shader input
		...
		TYA : STA !SpriteDir,x					;/
		LDA #$04 : STA !SpriteBlocked,x				; start on ground
		LDY !Difficulty						;\ HP
...
; debug: frame control
;	LDA $3280,x : STA !SpriteAnimIndex,x
...
;+	JMP GRAPHICS
	...
	+	JMP GRAPHICS						;/
		...
		STZ !SpriteBlocked,x					; > remove collision
		JMP MAIN_RunPtr						;/
		..alive
	...
	INTERACTION:
		...
		LDA !SpriteBlocked,x					;\ check air/ground
		AND #$04 : BEQ ..air					;/
		..ground						;\
	...
	PrepChop:
		...
		JSL PlayerContact : BCS .StartChop			;/
		RTS							; return

		.StartChop						;\
		LDA #$01 : STA !CW_Attack,x				; |
		LDA #$22 : STA !CW_AttackTimer,x			; | go to chop
		LDA #!CW_Chop : STA !SpriteAnimIndex,x			; |
		STZ !SpriteAnimTimer,x					;/
	...
	Jump:
		...
		LDA #!CW_Jump : STA !SpriteAnimIndex,x			; |
		STZ !SpriteAnimTimer,x					; |
	...
	HurricaneJump:
		...
		.Init							; |
		ORA #$80 : STA !CW_Attack,x				; |
		LDA #!CW_SpinJump : STA !SpriteAnimIndex,x		; |
		STZ !SpriteAnimTimer,x					; | init speed + anim
	...
	Hurricane:
		...
		LDA #!CW_Spin : STA !SpriteAnimIndex,x			; |
		STZ !SpriteAnimTimer,x					; |
		.Main							;/
		LDY !SpriteDir,x					;\ X speed while descending
	...
	Stuck:
		...
		.Init							; |
		ORA #$80 : STA !CW_Attack,x				; | init anim
		LDA #!CW_Stuck : STA !SpriteAnimIndex,x			; |
		STZ !SpriteAnimTimer,x					; |
		.Main							;/
		STZ !SpriteXSpeed,x					; X speed = 0
		RTS							; return
	...
	HurtGround:
		...
		.Init							; | init anim
		LDA #!CW_Hurt : STA !SpriteAnimIndex,x			; |
		STZ !SpriteAnimTimer,x					; |
		.Main							;/
		LDA !SpriteBlocked,x					;\ return if in midair
		AND #$04 : BEQ .Return					;/
		...
		STZ $00							; |
		LDA #$0C : STA $01					; |
		LDA #!prt_smoke8x8 : JSL SpawnParticle_NoSpeedAcc	;/
	...
	HurtAir:
		...
		LDA #!CW_Hurt+1 : STA !SpriteAnimIndex,x		; |
		STZ !SpriteAnimTimer,x					; |
		.Main							;/
		LDA !SpriteBlocked,x					;\
		AND #$04 : BEQ .Return					; |
	...
	PrepPhase2:
		...
		STZ !CW_Attack,x
		LDA #!CW_Channel : STA !SpriteAnimIndex,x
		STZ !SpriteXSpeed,x
		.Main
	...
	-	STA !SpriteDisP1,x					; |
		...
		LDA $3200,x						; |
		CMP #$02 : BNE +					; |
		STZ !SpriteDir,x					; > all rex move right
		STZ !RexChase,x						; |
		STZ !ExtraProp1,x					; |
	+	DEX : BPL -						;/
		...
		LDA #$80 : STA !SPC3					; |
		LDA #$60 : STA !CW_AttackTimer,x			; |
		LDA !SpriteBlocked,x					; | init anim + music
		AND #$04 : BNE +					; |
	...
	+	STZ !CW_Attack,x					; |
		...
		LDA #!CW_Defeat : STA !SpriteAnimIndex,x		; |
		STZ !SpriteAnimTimer,x					; |
		STZ !SpriteXSpeed,x					;/
	...
	+	ORA #$02						; |
		...
		LDA .MinionTable+$10,y : STA !SpriteDir,x		; dir
