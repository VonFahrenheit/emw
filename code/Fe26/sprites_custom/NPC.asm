...
;
	...
	MAIN:
		...
		..setanim					;\
		STA !SpriteAnimIndex,x				; | set anim reg
		...
		STZ !SpriteOAMProp,x						; unload palset if character not visible
		LDA #$01 : STA !SpriteStatus,x					; return to INIT
		TXA								;\
		ASL A								; |
		TAX								; |
	...
	++	STA !SpriteAnimIndex,x
		...
		TAY						;/
		LDA #$00					; reset timer
	+	STA !SpriteAnimTimer,x				; update anim timer
		REP #$20					;\
		...
		LDA !SpriteAnimIndex,x : STA !PrevAnim,x
	...
	Leeway:
		...
		LDA .AnimTable+3,y : STA !SpriteAnimIndex,x	;\
		REP #$30					; |
	...
	.Movement
		...
		EOR #$FF : INC A
		STA !SpriteVectorX,x
		LDA #$01 : STA !SpriteVectorTimeX,x
	...
	DATA:
		...
		.VictoryPose
		db !Mar_Victory,!Lui_Victory,!Kad_Victory,!Lee_Victory,$00,$00
	...
	Sparkles:
		...
		STA $00						; |
		LDA !RNG					; |
		LSR #4						; | handle sparkle timer, even when not in play
		ORA #$08 : STA $01				; | (the animation plays while the player is in swap stun)
		STZ $02						; |
		LDA #$F0 : STA $03				; |
		STZ $04						; |
		LDA #$E8 : STA $05				; |
...
; if return with z = 1, this NPC should be disabled
	...
	Spy:
		...
		REP #$20
		LDA.w #.Sightbox : JSL LOAD_HITBOX
		JSL PlayerContact : BCC .CheckBush
		...
		CMP #$0001 : BNE ..next
		PHX
		LDA $410000+!BG_object_X,x : STA $E0
		LDA $410000+!BG_object_Y,x : STA $E2
	...
	Unload:
		...
		LDA !SpriteXLo,x : STA !P2XPosLo-$80,y		;\
		LDA !SpriteXHi,x : STA !P2XPosHi-$80,y		; | set coords
		LDA !SpriteYLo,x : STA !P2YPosLo-$80,y		; |
		LDA !SpriteYHi,x : STA !P2YPosHi-$80,y		;/
		...
		STA !P2YSpeed-$80,y				;/
		LDA !SpriteStatus,x : STA !P2Direction-$80,y	; dir
