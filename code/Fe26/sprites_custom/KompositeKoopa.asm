...
KompositeKoopa:
	...
	!KK_Player1Hurt =	$3280,x
	!KK_Player2Hurt =	$3290,x

	!KK_Player1Touch =	$32A0,x		; A player can only interact with 1 segment per frame
	!KK_Player2Touch =	$32B0,x		; If both are set, then it skips to head interaction

	!KK_OpenMouth =		$32D0,x		; While timer is non-zero, mouth is open

	!KK_HurtTimer =		$3300,x		; Flashing and invulnerable while non-zero

	!KK_DeathTimer =	$3340,x		; Used for death animation
...
; Assume 9 (2 for head/torso segment) tiles total.
	...
	Interact:
		LDA !KK_Player1Touch
		BEQ $03 : DEC !KK_Player1Touch
		LDA !KK_Player2Touch
		BEQ $03 : DEC !KK_Player2Touch
		STZ !KK_Player1Hurt
		STZ !KK_Player2Hurt
	...
	.Head	LDY #$00 : JSR .GetSegment
		LDA #$14 : STA $06
		JSL PlayerContact : BCC .CheckAttack
		LSR A : BCC ..P2
	..P1	LDY !KK_Player1Touch : BNE ..P2
		PHA
		LDA #$08 : STA !KK_Player2Touch
		LDY #$00 : JSR .CheckContact
		BCC $03 : INC !KK_Player1Hurt
		LDA !P2YSpeed-$80 : BMI ...End
		JSR .Stomp
	...End	PLA
	..P2	LDY !KK_Player2Touch : BNE .CheckAttack
		LSR A : BCC .CheckAttack
		LDA #$08 : STA !KK_Player2Touch
		LDY #$80 : JSR .CheckContact
		BCC $03 : INC !KK_Player2Hurt
	...
	-	JSR .GetSegment
		...
		PHY
		JSL PlayerContact : BCC .Nope
	..P1	LDY !KK_Player1Touch : BNE ..P2
		LSR A : BCC ..P2
		PHA
		LDY #$00 : JSR .CheckContact
		BCC $03 : INC !KK_Player1Hurt
		LDA #$08 : STA !KK_Player1Touch
		LDA !P2YSpeed-$80 : BMI ...End
		JSR .Bounce
	...End	PLA
	..P2	LDY !KK_Player2Touch : BNE .Nope
		LSR A : BCC .Nope
		LDY #$80 : JSR .CheckContact
		BCC $03 : INC !KK_Player2Hurt
		LDA #$08 : STA !KK_Player2Touch
	...
	.Nope	PLA
		...
		CMP #$04 : BEQ .Return
		TAY
		LDA !KK_Player1Touch : BEQ -
		LDA !KK_Player2Touch : BEQ -
	...
	.Stomp
		JSL P2Bounce
	..2	LDA !KK_HurtTimer : BEQ ..Yes
		LDA #$02 : STA !SPC1
		RTS

		..Yes
		LDA #$20 : STA !KK_OpenMouth
		LDA #$28 : STA !SPC4
		LDA #$80 : STA !KK_HurtTimer
		DEC !KK_HP
		RTS
	...
	GRAPHICS:
		LDY !SpriteAnimIndex				;\
		LDA !KK_DeathTimer : BNE +				; > Don't update position during death
		LDA !SpriteAnimTimer				; |
		INC A						; |
		CMP.w .AnimTime,y				; |
	...
	-	LDA !BigRAM+4,y					; |
		...
		.Draw
		LDA !KK_DeathTimer : BNE .Open			; mouth open during death animation
		LDA !KK_OpenMouth : BEQ .Closed
	.Open	LDA !BigRAM+5
		CLC : ADC #$03
		STA !BigRAM+5
		LDA !BigRAM+9
