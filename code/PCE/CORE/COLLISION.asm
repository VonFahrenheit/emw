...
COLLISION:
	...
	..w	STA !P2DropDownTimer			;/
		...
		.CollisionBorders
		BIT !HardBoxBorders : BPL ..levelborders
		BIT !CameraBoxU+1 : BMI ..levelborders

		..boxborders
		REP #$20
		LDA !CameraBoxL : STA $E0		; min X
		LDA !CameraBoxR				;\
		ORA #$00F0				; | max X
		STA $00					;/
		LDA !CameraBoxU : STA $E2		; min Y
		LDA !CameraBoxD				;\
		CLC : ADC #$00D0			; | max Y
		STA $02					;/
		BRA ..done
...
;	80 -> 10 (water center)
	...
	++	STA !P2YSpeed
		...
		LDA !P2Water				;\
		AND #$10 : BEQ .NoWaterFlag		; | center water bit marks water physics
		LDA #$40 : TSB !P2Water			; |
		.NoWaterFlag				;/
...
; scratch RAM:
; $00 - max X coord of area to allow collision level (usually level width)
; $02 - max Y coord of area to allow collision level (usually level height)
...
; $E0	min X coord of area to allow collision (usually 0)
; $E2	min Y coord of area to allow collision (usually 0)
...
macro SetCoords()
		...
		BCC $03 : ORA #$FF00			; |
		CLC : ADC !P2X				; |
		BPL $03 : LDA #$0000			; > not out of bounds negative
		CMP $00					; |
		BCC $02 : LDA $00			; > not out of bounds right
		CMP $E0					; |
		BCS $02 : LDA $E0			; > not out of bounds left
		STA $9A					; |
		LDA [$F3],y				; |
		AND #$00FF				; |
		CMP #$0080				; |
		...
		CMP $E2					; |
		BCS $02 : LDA $E2			; > not out of bounds up
		STA $98					; |
		SEP #$20				;/
endmacro
...
endmacro
	...
	GET_TILE:
		...
		SEP #$20				; |
		BCC .Not3DWater				; |
		LDA !IceLevel : BEQ .3DWater		; | apply 3D water/ice
		REP #$20				; |
		LDA #$0130 : STA $0C			; |
		BRA .SetIndex				; |
		.3DWater				; |
		LDA .CollisionTable,y : TSB !P2Water	; |
		.Not3DWater				;/
...
; - 1D7: 1/1 slope right
		...
		LDX $0C						; |
		CPX #$71 : BCC ..nowater			; |
		CPX #$B4 : BCS ..nowater			; | water slope
