	...
	LAYER_INTERACTION:		;CODE_019140:
		...
		STZ $7694
		LDA !P2ExtraBlock			;\
		AND #$7F				; | only applies for 1 frame
		STA !P2Blocked				;/
		STZ !P2Slope
		STZ $785E
		STZ $785F
		...
		LDA !P2ExtraBlock			;\
		LSR A					; | write external water status
		AND #$40				; |
		TSB !P2Water				;/
		STZ !P2ExtraBlock			; only applies for 1 frame
...
;		STA $0D
	...
	..40	AND #$3FFF
		ASL A
		TAY
		LDA !Map16ActsLike40+0 : STA $00
		LDA !Map16ActsLike40+1
		ORA #$0080			; lunar magic does not set this bit... for some reason
		STA $01
	...
	MidwayPoint:
		...
		LDX !Translevel				;\
		LDA !LevelTable1,x			; |
		AND.b #$20^$FF				; | set flag in OW table
		ORA #$40				; |
		STA !LevelTable1,x			;/
		LDA !Level : STA !LevelTable2,x		;\
		LDA !Level+1 : BEQ +			; |
		LDA !LevelTable1,x			; | store level number
		ORA #$20				; |
		STA !LevelTable1,x			;/
	+	LDA #$05 : STA !SPC1
		LDA !P2HP
	...
	YoshiCoin:
		...
		LDA !CurrentPlayer		;\
		TAX				; |
		LDA !P1CoinIncrease,x		; | Increase coins (200)
		CLC : ADC #$C8			; |
	...
	-	LDA !Ex_Num,y : BEQ .Spawn
		...
		STA !Ex_XHi,y			;/
		LDA #$07+!MinorOffset : STA !Ex_Num,y
...
BOUNCE_SPRITE:
		...
		LDY.b #!Ex_Amount-1		; Highest bounce sprite index

		.Loop
		LDA !Ex_Num,y : BEQ .Spawn	;\
		DEY : BPL .Loop			; | Get bounce sprite number
		RTS				;/

		.Spawn
		STA !Ex_Data1,y			; > Layer 1, going up
		STA !Ex_XSpeed,y		; > X speed
		LDA #$C0			;\ Y speed
		STA !Ex_YSpeed,y		;/
		...
		TYX				;\
		DEX : BMI .Quake		; |
	-	LDA !Ex_Num,x : BEQ .Quake	; | find an extra index for quake sprite
		DEX : BPL -			; |
		LDA #$01 : STA !Ex_Num,x	; \ quake sprite data
		LDA #$06 : STA !Ex_Data1,y	; /
		.Quake				;/

		LDA $9C				;\ Map16 tile bounce sprite should turn into
		STA !Ex_Data2,y			;/
		LDA $7C				;\
		CLC : ADC.b #!BounceOffset	; | bounce sprite number
		AND #$7F			; > init routine
		STA !Ex_Num,y			;/
