...
; VON FAHRENHEIT'S EXTRA SRAM PATCH
;
; I made this patch specfically to work with the SP pack, but it should work on it's own, too.
; It haven't tested yet, but it will almost certainly NOT work with MarioE's SRAM plus, since SRAM plus overwrites
; the SRAM transfer routines.
;
; --Features--
;
; - Adds 256 extra bytes of SRAM for each file, starting at $700400.
; - Let's you choose a difficulty when creating a new file. Value is located at $7FA301 during gameplay.
; - Disables SMW's 2-player mode. It's code is overridden by my difficulty setting code.
; - Stores number of Yoshi Coins collected at $7FA303. Goes up to 65535 (0xFFFF).
; - Stores a death counter for player 1 at $7FA305. Goes up to 65535 (0xFFFF).
; - Gives each Yoshi coin it's own bit in the 96 (0x60) byte table at $7FA309. If the corresponding bit is set,
;   the Yoshi Coin won't spawn.
; - Changes the status bar to work with the SP pack.
; - Removes useless animations for powerups. Also doesn't pause the game when Mario gets hurt.
...
; $0FFE7E
	...
	org $0FFE87
		JSL BG3Fix				; Source: STY $4322 : SEP #$20 (inserted by Lunar Magic)
		NOP					;
	...
	STATUS_BAR:
		...
		LDA !Difficulty				;\
		AND #$10				; | Only use timer during Crisis Mode
		BEQ .Coins				;/
		LDA $7493				;\
...
.MarioItem	db $FC,$95,$97,$96,$FC			; Nothing, mushroom, flower, star, feather (nothing)
...
.YoshiCoinGFX:	db $27,$2E				; Not collected, collected
...
;=======================;
	...
	org $028012
		BRA $03					;\ No SFX when swapping powerups
		NOP #3					;/

	org $028042					; Code that handles powerup from item box

		PLY					; > Y = item index
		LDA.w .Status,y				;\
		LDY $19					; |
		STA $19					; | Swap powerups
		LDA.w .Box,y				; |
		STA $6DC2				;/
		BNE +					; > Ignore sound when swapping
		LDA #$0A : STA !SPC1			; > Powerup sound
	+	PLX					;\ Return
		RTL					;/
...
org $05B34A		; This routine give coins from blocks and such
	JSL BetterCoins	;\
...
;freecode
	...
	BG3Fix:
		...
		PHX
		PHY
		CMP #$3981 : BEQ .Normal		; download works as usual
		CPX #$5000 : BEQ .Fix			;\ 0x5000 and 0x50A0 warrant a backup of the top row
		CPX #$50A0 : BNE .Normal		;/

		.Fix
		PHP					;\
		REP #$20				; |
		LDX #$003E				; | store dynamic BG3
	-	LDA $7EAD00,x : STA $0020,x		; |
		DEX #2 : BPL -				; |
		PLP					;/
		LDX #$5000 : STX $2116			;\ Force the game to load the entire BG3 tilemap
		CPY #$AE40 : BNE .Normal		; > Don't mess with $BD00!
		LDY #$AD00				;/
